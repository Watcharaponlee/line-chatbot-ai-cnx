      ...
      case "message":
        if (event.message.type === "text") {
          ...
          switch (event.message.text) {
            ...
            case "profile":
              await line.replyWithStateless(event.replyToken, [
                flex.profile(profile.pictureUrl, profile.displayName),
              ]);
              break;
            case "vdo":
        ...
        await line.reply(event.replyToken, [
          {
            ...
            sender: {
              name: "BOT",
              iconUrl:
                "https://cdn-icons-png.flaticon.com/512/10176/10176915.png ",
            },
            quickReply: {
              items: [
                {
      ...
      case "message":
        if (event.message.type === "text") {
          ...
          if (event.message.text == "me") {
            const profile = await line.getProfileByGroup(
  ...
  for (const event of events) {
    switch (event.type) {
      case "message":
        if (event.message.type === "text") {
          if (event.message.text === "flex") {
            await line.replyWithStateless(event.replyToken, [
              flex.unixtime(currentTimeStamp + 60),
            ]);
          }
        }
      ...
      case "postback":
        ...
        let message =
          currentTimeStamp > expireTime ? "❌ Time Out" : "✅ In Time";
